                    ...
                    @elseif(isset($menu['text']))
                        ...
                        @elseif ($route || Route::has($route))
                            @php
                                ...
                                $is_active = Route::currentRouteName() === $route ? 'active' : '';
                            @endphp
                        ...
                        <li class="sidebar-item {{ $is_active }} @isset($menu['submenu']) has-sub @endisset">
                            ...
                            @isset($menu['submenu'])
                                <ul class="submenu {{ $submenu_is_active ? 'active submenu-open' : '' }}">
                                    @foreach ($menu['submenu'] as $submenu)
                                        @isset($submenu['can'])
                                            @continue(!auth()->user()?->can($submenu['can']))
                                        @endisset
                                        @php
                                            $url = Route::has($submenu['route']) ? route($submenu['route']) : '#';
                                            $is_active = Route::currentRouteName() === $submenu['route'] ? 'active' : '';
                                        @endphp

                                        <li class="submenu-item {{ $is_active }}">
                                            <a href="{{ $url }}" class="submenu-link">{{ $submenu['text'] }}</a>
                                        </li>
                                    @endforeach
                                </ul>
                            @endisset
